<template>
	...
	<view>
		<view class="ordering-fls">
					...
					<block v-for="(item,index) in classifarr" :key="index">
						<view class="content-view">
							...
							<view class="content-img">
								<image src="../../../static/img/mifan.jpg" mode="aspectFill"></image>
							</view>
							<!-- 右边商品 -->
							<view class="content-title">
								<text class="conteng-take">{{item.name}}</text>
								<view class="conteng-monthly">
									<!-- 									<block v-for="(itemdata,index) in item.tag" :key="index">
										<text></text>
									</block> -->
								</view>
								...
								<view class="conteng-price">
									...
									<view class="conteng-hua">¥100</view>
									<view class="ordering-price">
										<image @click="minus(item.amount,item.id,index,item.price)" src="../../../static/img/minus.svg" mode="widthFix"></image>
										<text class="amounting">{{item.amount}}</text>
										<image @click="plus(item.amount,item.id,index,item.price)" src="../../../static/img/plus.svg" mode="widthFix"></image>
									</view>
								</view>
							</view>
						</view>
			...
			<view class="Shopping">
				<view class="qishou">
					<image v-if="!isActive" src="../../../static/img/0.svg" mode="widthFix"></image>
					<image v-if="isActive" src="../../../static/img/1.svg" mode="widthFix"></image>
				</view>
				<!-- 多少量 -->
				<view class="Numbering" v-if="isActive">{{Takeleng}}</view>
				<!-- 金额 -->
				<view class="Shopping-dis">
					<view class="Shopping-left">
						<view class="Cost-mony">
							<text class="Total-price">¥{{Takeawayprice}}</text>
							<text class="Delivery">另需配送费10元</text>
						</view>
					</view>
					<view class="Shopping-right" :class="{shoppactive: isActive}" @click="flat && settLement()">
						<text v-if="!isActive">5元起送</text>
